...
const AssetList = () => {
  ...
  const checkServerConnection = async () => {
    try {
      const response = await axios.get(`${process.env.API_URL}/test`);
  ...
  useEffect(() => {
    const fetchSortedAssets = async () => {
      if (sortCriteria === 'activeFirst' || sortCriteria === 'inactiveFirst') {
        try {
          const response = await axios.get(`${process.env.API_URL}/api/assets/sorted?sortOrder=${sortCriteria}`);
  ...
  const fetchLocations = async () => {
    try {
      const response = await axios.get(`${process.env.API_URL}/api/locations`);
  ...
  const fetchTotalActiveAssets = async () => {
    try {
      const response = await axios.get(`${process.env.API_URL}/api/assets/active/count`);
  ...
  const handleDeleteAsset = useCallback(async (assetId) => {
    try {
      ...
      const response = await axios.delete(`${process.env.API_URL}/api/Assets/delete/${assetId}`);
  ...
  const updateAssetQuantity = useCallback(async (assetId, newQuantity) => {
    try {
      await axios.put(`${process.env.API_URL}/api/Assets/updateQuantity/${assetId}`, {
  ...
  useEffect(() => {
    const eventSource = new EventSource(`${process.env.API_URL}/api/assets/sse`);
