...
const Modal = ({ isOpen, onClose, onSaveCategory, categories, onDeleteCategory }) => {
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.API_URL}/api/categories`, { categoryName: category });
  ...
  const handleDeleteCategory = async (cat) => {
    try {
      await axios.delete(`${process.env.API_URL}/api/categories/${encodeURIComponent(cat)}`);
